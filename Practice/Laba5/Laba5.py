...
#  ШАГ 7: Обработка видео
print("\nШаг 7: Обработка видео (cars.mp4)...")


def process_video(input_path, output_path=None):
    """Обработка видео: ч/б + Canny края"""
    mp4 = cv2.VideoCapture(input_path)
    ...
    fps = int(mp4.get(cv2.CAP_PROP_FPS))
    ...
    while True:
        ...
        # Объединяем оригинал, ч/б и края
        combined = np.hstack([frame, gray_bgr, edges_bgr])

        # Отображение
        cv2.imshow("Video Processing", combined)
...
if not cap.isOpened():
    ...
    while n != 5:
        ...
        if not ret:
            print("Достигнут конец видео или произошла ошибка.")
            break
        # Отображение кадра в отдельном окне
        cv2.imshow(frame)

        n += 1

        # # Ожидание нажатия клавиши для выхода
        # key = cv2.waitKey(30)  # Задержка в 30 мс
        # if key == 27:  # Esc
        #     break
...
# Запуск обработки видео (раскомментируйте, если есть файл cars.mp4)
